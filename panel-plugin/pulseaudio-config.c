...
#define DEFAULT_SHOW_MICROPHONE                   FALSE
...
pulseaudio_config_class_init (PulseaudioConfigClass *klass)
{
  ...
  g_object_class_install_property (gobject_class,
                                   PROP_SHOW_MICROPHONE,
                                   g_param_spec_boolean ("show-microphone", NULL, NULL,
                                                         DEFAULT_SHOW_MICROPHONE,
                                                         G_PARAM_READWRITE |
                                                         G_PARAM_STATIC_STRINGS));
...
pulseaudio_config_init (PulseaudioConfig *config)
{
  ...
  config->show_microphone           = DEFAULT_SHOW_MICROPHONE;
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SHOW_MICROPHONE:
      g_value_set_boolean (value, config->show_microphone);
      break;
                                ...
                                GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SHOW_MICROPHONE:
      val_bool = g_value_get_boolean (value);
      if (config->show_microphone != val_bool)
        {
          config->show_microphone = val_bool;
          g_object_notify (G_OBJECT (config), "show-microphone");
          g_signal_emit (G_OBJECT (config), pulseaudio_config_signals [CONFIGURATION_CHANGED], 0);
        }
      break;
...
gboolean
pulseaudio_config_get_show_microphone (PulseaudioConfig *config)
{
  g_return_val_if_fail (IS_PULSEAUDIO_CONFIG (config), DEFAULT_SHOW_MICROPHONE);

  return config->show_microphone;
}
  ...
  if (xfconf_init (NULL))
    {
      ...
      property = g_strconcat (property_base, "/show-microphone", NULL);
      xfconf_g_property_bind (channel, property, G_TYPE_BOOLEAN, config, "show-microphone");
      g_free (property);
