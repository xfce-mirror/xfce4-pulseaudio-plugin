  ...
  if (event->state & GDK_CONTROL_MASK)
    {
      ...
      if (g_list_length (sinks) > 1)
        {
          const gchar    *default_output_name = pulseaudio_volume_get_default_output(button->volume);
          GList          *found = g_list_find_custom(sinks, default_output_name, (GCompareFunc)g_strcmp0);
          ...
          if (found != NULL)
            {
              if (event->direction == 1)  // next output
                ...
                {
                  found = g_list_previous(found);
                  if (found == NULL)
                    found = g_list_last(sinks);
                  change = TRUE;
                }
              if (change)
                pulseaudio_volume_set_default_output (button->volume, (gchar *)found->data);
            }
        }
      g_list_free (sinks);
    }
